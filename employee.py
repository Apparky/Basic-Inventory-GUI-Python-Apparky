...
customer_new_bill = StringVar()
customer_search_bill = StringVar()
bill_date = StringVar()
...
def logout():
    log_out = messagebox.askyesno("Logout", "Are you sure you want to logout?", parent=biller)
    if log_out == True:
        biller.destroy()
        root.deiconify()
        page1.entry1.delete(0, END)
...
class login_page:
    def __init__(self, top=None):
        ...
        self.entry2.configure(textvariable=pass_)
...
class bill_window:
    def __init__(self, top=None):
        ...
        self.entry1.configure(relief="flat")
        self.entry1.configure(textvariable=customer_name)
        ...
        self.entry2.configure(relief="flat")
        self.entry2.configure(textvariable=customer_number)
        ...
        self.entry3.configure(textvariable=customer_search_bill)
    ...
    def gen_bill(self):
        ...
        if self.state == 1:
            ...
            if (customer_name.get() == ""):
            ...
            elif (customer_number.get() == ""):
                messagebox.showerror("Oops!", "Please enter a number.", parent=biller)
            elif valid_phone(customer_number.get()) == False:
                ...
                if strr.find('Total') == -1:
                    ...
                    self.name_message.insert(END, customer_name.get())
                    self.name_message.configure(state="disabled")

                    self.num_message.insert(END, customer_number.get())
                    self.num_message.configure(state="disabled")

                    customer_new_bill.set(random_bill_number(8))

                    self.bill_message.insert(END, customer_new_bill.get())
                    ...
                    cur.execute(insert, [customer_new_bill.get(), bill_date.get(), customer_name.get(), customer_number.get(),
    ...
    def search_bill(self):
        ...
        cur.execute(find_bill, [customer_search_bill.get().rstrip()])
        results = cur.fetchall()
        if results:
            self.clear_bill()
            self.wel_bill()
